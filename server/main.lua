...
local function validate(info, tbl)
    for k,v in pairs(tbl) do
        if v then
            if v.info and (info.firstname == v.info.firstname) and (info.lastname == v.info.lastname) then
                return true
            end
        end
    end
...
QBCore.Functions.CreateCallback('qb-shops:server:validate license', function(source,cb) 
    local src = source
    local Player = QBCore.Functions.GetPlayer(src) 
    local weaponlicenses = exports['qb-inventory']:GetAllItemsByName(src, 'weaponlicense')
    local idcards = exports['qb-inventory']:GetAllItemsByName(src, 'id_card')
    if weaponlicenses then 
        for k,v in pairs(weaponlicenses) do
            if v then
                local val = validate(v.info, idcards)
                if val then
                    cb(val)
                end
            end
